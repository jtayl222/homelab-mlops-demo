...
DEPLOYMENT_NAME=$(kubectl get seldondeployments -n iris-demo -o name | head -1 | cut -d'/' -f2)

if [ ! -z "$DEPLOYMENT_NAME" ]; then
  # Port forward and test prediction
  kubectl port-forward -n iris-demo svc/${DEPLOYMENT_NAME}-default 8080:8080 &
  sleep 5
