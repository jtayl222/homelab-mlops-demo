...
DEPLOYMENT_NAME=$(kubectl get seldondeployments -n argowf-dev -o name | head -1 | cut -d'/' -f2)

if [ ! -z "$DEPLOYMENT_NAME" ]; then
  # Port forward and test prediction
  kubectl port-forward -n argowf-dev svc/${DEPLOYMENT_NAME}-default 8080:8080 &
  sleep 5
